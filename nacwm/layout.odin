...
monitor_tile :: proc(monitor : Monitor) {
    ...
    for client, client_idx in g_clients {
        if client.floating || !client_is_visible_in_monitor(client_idx, monitor) do continue
        num_tiled += 1
    }
    ...
    master_idx : Client_Index
    master_width := i32(f32(monitor_size.x) * monitor.master_factor)
    #reverse for &client, client_idx in g_clients {
        if client.floating || !client_is_visible_in_monitor(client_idx, monitor) do continue
    ...
    #reverse for &client, client_idx in g_clients[:master_idx] {
        if client.floating || !client_is_visible_in_monitor(client_idx, monitor) do continue
